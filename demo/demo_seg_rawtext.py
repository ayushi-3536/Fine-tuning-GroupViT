...
from segmentation.evaluation import build_seg_demo_pipeline, build_seg_inference, build_seg_inference_for_customclasses
...
def parse_args():
    ...
    parser.add_argument(
        '--text', default=[ 'background', 'woman'], help='dataset classes for visualization')

    parser.add_argument('--input', default='demo/examples/coco.jpg', type=str, help='input image path')
    parser.add_argument('--output_dir', default='demo/output/openvocabss', type=str, help='output dir')
...
def inference(args, cfg):
    ...
    seg_model = build_seg_inference_for_customclasses(model, dataset_class, text_transform, cfg.evaluate.seg, args.text)

    #append the list of text to the output dir by concatenating all string as _
    args.output_dir = osp.join(args.output_dir, '_'.join(args.text))
