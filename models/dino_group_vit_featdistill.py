...
def co_pca_dino_gvit(features, target_dim=384):
    ...
    print("before centered_features.dtype", centered_features.dtype)
    centered_features = centered_features.float()
    print("after centered_features.dtype", centered_features.dtype)
...
class GroupingBlock(nn.Module):
    ...
    def __init__(self,
        ...
        ######print('num_output_group', num_output_group)
    ...
    def project_group_token(self, group_tokens):
        ...
        """
        # [B, S_2, C] <- [B, S_1, C]
        #check device of group_tokens and self.mlp_inter\
    ...
    def forward(self, x, group_tokens, return_attn=False):
        ...
        ######print('inside grouping block group_tokens', group_tokens.shape)
        x = self.norm_x(x)
        ######print('inside grouping block x', x.shape)
        # [B, S_2, C]
        projected_group_tokens = self.project_group_token(group_tokens)
        ######print('inside grouping block projected_group_tokens', projected_group_tokens.shape)
        projected_group_tokens = self.pre_assign_attn(projected_group_tokens, x)
        ######print('inside grouping block projected_group_tokens after pre_assign_attn', projected_group_tokens.shape)
        new_x, attn_dict = self.assign(projected_group_tokens, x, return_attn=return_attn)
        ######print('inside grouping block new_x', new_x.shape)
        new_x += projected_group_tokens
        #######print('inside grouping block new_x after projected_group_tokens', new_x.shape)

        new_x = self.reduction(new_x) + self.mlp_channels(self.norm_new_x(new_x))
        #######print('inside grouping block new_x after reduction', new_x.shape)
...
class GroupingLayer(nn.Module):
    ...
    def set_dino_features(self, dino_features):
        self.dino_features = dino_features

    def reset_dino_features(self):
    ...
    def forward(self, x, prev_group_token=None, return_attn=False):
        ...
        B, L, C = x.shape
        #######print("inside grouping layer x shape is ", x.shape)
        #put x and group_token on the same device
        if group_token is not None:
            group_token = group_token.to(x.device)
            #######print("in grouping lyr group token shape is ", group_token.shape)
        ...
        ######print(" in grouping layer after split x shape is ", x.shape)
        
        attn_dict = None
        if self.dino_features is not None:
            print("in grouping layer, dino features shape is ", self.dino_features.shape)
            #l2 normalize x and dino_features before concatenating
            x = F.normalize(x, p=2, dim=-1)
            self.dino_features = F.normalize(self.dino_features, p=2, dim=-1)
            print("Fusing dino features with groupViT features")
            #x features are of shape [B,N,D1] and dino features are of shape [B,N,D2], concatenate them such that the final shape is [B,N,T] where T = 0.5*D1 + 0.5*D2
            #scale dino features by 0.5
            #self.dino_features = 0.5*self.dino_features

            #scale x features by 0.5 along last dimension
            #x = 0.5*x
            #concatenate x and dino features


            #add dino features to x along last dimension
            # alpha = 0.5
            # x = alpha * x + (1-alpha) * self.dino_features

        
            print("x dtype before ", x.dtype)
            x_fused = torch.cat([x, self.dino_features], dim=-1)
            print("after concatenating dino features, x shape is ", x_fused.shape)


            #Use PCA to Project the concatenated features to the original dimension
            
            x = co_pca_dino_gvit(x_fused.float(), target_dim=x.shape[-1])
            print("after PCA, x shape is ", x.shape)
            print("x dtype after ", x.dtype)
            

            # print("after concatenating dino features, x shape is ", x.shape)
        if self.downsample is not None:
            device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
            x = x.to(device)
            group_token = group_token.to(device)
            x, attn_dict = self.downsample(x, group_token, return_attn=return_attn)
            ######print("in grouping layer after downsample x shape is ", x.shape)
            ######print("in grouping layer after downsample group token shape is ", group_token.shape)
...
class DINOFeatDistillGroupViT(nn.Module):
    ...
    def __init__(self,
        ...
        DINO_STRIDE=16
        self.extractor = ViTExtractor(dino_model, DINO_STRIDE)
        patch_size = self.extractor.model.patch_embed.patch_size
        
        num_patches = int(patch_size / DINO_STRIDE * (img_size // patch_size - 1) + 1)
       
            
        #patches_resolution = extractor.model.patch_embed.patches_resolution
        ...
        #change first layer to use dino extractor
        for i_layer in range(self.num_layers):

            dim = int(embed_dim * embed_factors[i_layer])
            ######print(f"dim of layer:{i_layer} is {dim}")
            downsample = None
            if i_layer < self.num_layers - 1:
                out_dim = embed_dim * embed_factors[i_layer+1]
                ######print(f"out_dim of layer:{i_layer} is {out_dim}")
                ######print(f"num_output_groups of layer:{i_layer} is {num_output_groups[i_layer]}")
                ######print(f"num_group_tokens of layer:{i_layer} is {num_group_tokens[i_layer]}")
    ...
    def forward_features(self, x, *, return_attn=False, concat_dino=False):
        img_tensor=x
        ...
        x = self.pos_drop(x)
        print("x shape after pos drop", x.shape)
        #squeeze desc dim from [B, 1, N, D] to [B, N, D]
        #batch_desc_dino = batch_desc_dino.squeeze(1)       

        group_token = None
        attn_dict_list = []
        
        #stack ectracted dino features with x
        if concat_dino:
            batch_desc_dino = self.extractor.extract_descriptors(img_tensor.cuda(), 9, 'key')
            print("batch_desc_dino shape", batch_desc_dino.shape)
            #x = torch.cat((x, batch_desc_dino), dim=1)
        print("x shape", x.shape)
        print("concat dino features", concat_dino)
        for idx, layer in enumerate(self.layers):
            #check device for batch_desc_dino and group_token amd move to cuda if needed
            
            if group_token is not None:
                group_token = group_token.to(x.device)
            if idx == 0 and concat_dino:
                layer.set_dino_features(batch_desc_dino)
                print("set dino features done")
                x, group_token, attn_dict =  layer(x, group_token, return_attn=return_attn)
                layer.reset_dino_features()
            else:
                x, group_token, attn_dict = layer(x, group_token, return_attn=return_attn)
    ...
    def forward(self, x, *, return_feat=False, return_attn=False, as_dict=False, concat_dino=False):
        print("x shape", x.shape)
        x, group_token, attn_dicts = self.forward_features(x, return_attn=return_attn, concat_dino=concat_dino)
