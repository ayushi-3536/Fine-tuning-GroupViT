...
class AssignAttention(nn.Module):
    ...
    def forward(self, query, key=None, *, value=None, return_attn=False):
        ...
        # print("raw attn shape", raw_attn.shape)
        ...
        if return_attn:
            ...
            #take last two dimensions of query [1, 1, G, D], keep just [G,D]
            group_feat = query.squeeze(0).squeeze(0)
            #print("group feat shape", group_feat.shape)
            attn_dict = {'hard': hard_attn, 'soft': soft_attn, 'group_feat':group_feat}
        else:
            attn_dict = None
...
class GroupViT(nn.Module):
    ...
    def forward(self, x, *, return_feat=False, return_attn=False, as_dict=False):
        ...
        outs.append(x, name='before_avg_pool_x')
