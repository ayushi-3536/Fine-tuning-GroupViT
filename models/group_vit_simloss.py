...
class AssignAttention(nn.Module):
    ...
    def forward(self, query, key=None, *, value=None, return_attn=False):
        ...
        if return_attn:
            ...
            attn_dict = {'hard': hard_attn, 'soft': soft_attn, 'raw': raw_attn}
        else:
            attn_dict = None
...
class GroupingLayer(nn.Module):
    ...
    def forward(self, x, prev_group_token=None, return_attn=False, return_pre_downsampling_feature=False):
        """
        Args:
            x (torch.Tensor): image tokens, [B, L, C]
        ...
        #print("returning from grouping layer", return_pre_downsampling_feature)
        if return_pre_downsampling_feature:
            #print("returning")
            return x, group_token, attn_dict,  pre_downsample_feature
...
class GroupViT_SimLoss(nn.Module):
    ...
    def forward_features(self, x, *, return_attn=False):
        ...
        for idx,layer in enumerate(self.layers):
            ...
            if idx == 0:
                x, group_token, attn_dict, pre_downsample_features = layer(x, group_token, return_attn=return_attn,return_pre_downsampling_feature=True)
            else:
        ...
        return x, group_token, attn_dict_list, pre_downsample_features
    
    def get_forward_features(self, x, *, return_attn=False, concat_dino=False):
        img_tensor=x
        # print("img_tensor shape", img_tensor.shape)
        # print("x shape", x.shape)
        B = x.shape[0]
        ...
        x, group_token, attn_dict, feature_pre_downsampling =  layer(x, group_token, return_attn=return_attn)
        
        attn_dict_list.append(attn_dict)
        x = self.norm(x)

        return x, group_token, attn_dict_list, feature_pre_downsampling
    ...
    def forward(self, x, *, return_feat=False, return_attn=False, as_dict=False, inference=False):
        #print("x shape", x.shape)
        img_tensor=x
        x, group_token, attn_dicts, pre_downsample_feature = self.forward_features(x, return_attn=return_attn)
        ...
        if not inference:
            ...
            outs.append(batch_desc_dino, name='dino_feat')
            outs.append(pre_downsample_feature, name='pre_downsample_feat')
    ...
    def get_features(self, x, *, return_feat=False, return_attn=False, as_dict=False, concat_dino=False):
        #print("x shape", x.shape)
        img_tensor=x
        x, group_token, attn_dicts, feature_pre_downsampling = self.get_forward_features(x, return_attn=return_attn, concat_dino=concat_dino)
        x_feat = x if return_feat else None
        ...
        outs.append(feature_pre_downsampling, name='gvit_feat')
        outs.append(batch_desc_dino, name='dino_feat')
