...
class MultiLabelContrastiveCELoss(nn.Module):
    ...
    def simloss(self, soft_attn, dino_dist_mat):
        max_indices = torch.argmax(soft_attn, dim=1)
        # Convert the indices to one-hot vectors
        onehot_prob_tensor =  rearrange(F.one_hot(max_indices, num_classes=soft_attn.shape[1]).to(dtype=soft_attn.dtype), 'b n d -> b d n')
        updated_prob = soft_attn * onehot_prob_tensor
    ...
    def softcrossentropy(self, gvit_sim, dino_sim):
        dino_sim_prob = F.softmax(dino_sim, dim=-2)
        #dimension against dim=-2

        #dino_sim_prob = F.normalize(dino_sim, dim=-1)
        #gvit_sim = F.normalize(gvit_sim, dim=-1)
        output = self.soft_cross_entropy(gvit_sim, dino_sim_prob)
        return output

    def KLloss(self, gvit_sim, dino_sim):
        loss = self.KL_div(gvit_sim, F.softmax(dino_sim, dim=-1))
        return loss
    ...
    def encode_image(self, image, *, return_feat=False, return_attn = True, as_dict=False, inference=False):
        outs = Result(as_dict)
        
        img_outs = self.img_encoder(image, return_feat=return_feat, return_attn=True, as_dict=True, inference=inference)
        #print(img_outs.keys())
        ...
        if not inference:
            outs.append(img_outs['dino_feat'], 'dino_feat')
            
            outs.append(img_outs['pre_downsample_feat'], 'pre_downsample_feat')
    ...
    def forward_train(self, image, text):
        ...
        # if 'dino_feat' in image_outs.keys():
        #     dino_feat =  image_outs['dino_feat']
        #     print("dino feat", dino_feat.shape)
        #     dino_sim_mat = chunk_cosine_sim(dino_feat, dino_feat)
        #     print("dino sim mat", dino_sim_mat.shape)
        #     # gvit_feat = image_outs['pre_downsample_feat']
        #     # gvit_sim_mat = chunk_cosine_sim(gvit_feat, gvit_feat)
            
        #     soft_attn = image_outs['image_attn'][0]['soft'].squeeze(1)
        #     print("soft attn", soft_attn.shape)

        #     soft_attn = torch.matmul(soft_attn.transpose(1,2), soft_attn)
        #     print("after trasnpose and self -multiplication soft attn", soft_attn.shape)
        #     # sim_loss = self.softcrossentropy(soft_attn, dino_sim_mat)
        #     # losses_dict['celoss_simmatrix'] = sim_loss
        #     # print("sim loss", sim_loss)
        #     klloss = self.KLloss(soft_attn, dino_sim_mat)
        #     losses_dict['klloss_simmatrix'] = klloss
        #     print("kl loss", klloss)

        if 'dino_feat' in image_outs.keys():
            dino_feat =  image_outs['dino_feat']
            #print("dino feat", dino_feat.shape)
            #torch.save(dino_feat, "batch_dino_feat.pt")
            dino_sim_mat = chunk_cosine_sim(dino_feat, dino_feat)
            # print("dino sim mat", dino_sim_mat.shape)
            # print("dino sim mat", dino_sim_mat)
            # gvit_feat = image_outs['pre_downsample_feat']
            # gvit_sim_mat = chunk_cosine_sim(gvit_feat, gvit_feat)
            
            raw_attn = image_outs['image_attn'][0]['raw'].squeeze(1)
            
            raw_attn_1 = image_outs['image_attn'][1]['raw'].squeeze(1)

            
            final_raw_attn = torch.matmul(raw_attn_1, raw_attn)
            final_raw_attn_1 = torch.matmul(final_raw_attn.transpose(1,2), final_raw_attn)
            #torch.save(raw_attn, "batch_raw_attn.pt")
            # print("raw attn", raw_attn.shape)
            # print("raw attn", raw_attn)

            soft_attn = image_outs['image_attn'][0]['soft'].squeeze(1)
            
            soft_attn_1 = image_outs['image_attn'][1]['soft'].squeeze(1)
            #print("soft attn 1", soft_attn_1.shape)
            final_soft_attn = torch.matmul(soft_attn_1, soft_attn)
            #print("final soft attn shape", final_soft_attn.shape)
            
            final_soft_attn_1 = torch.matmul(final_soft_attn.transpose(1,2), final_soft_attn)
            ce_loss = self.softcrossentropy(final_soft_attn_1, dino_sim_mat)
            losses_dict['softceloss_1'] = ce_loss
            #print("ce loss_1", ce_loss)

            #torch.save(soft_attn, "batch_soft_attn.pt")
            # print("soft attn", soft_attn.shape)
            # print("soft attn", soft_attn)

            # soft_attn = torch.matmul(soft_attn.transpose(1,2), soft_attn)
            # ce_loss = self.softcrossentropy(soft_attn, dino_sim_mat)
            # losses_dict['softceloss'] = ce_loss
            # print("ce loss", ce_loss)

            # raw_attn = torch.matmul(raw_attn.transpose(1,2), raw_attn)
            # ce_loss = self.softcrossentropy(raw_attn, dino_sim_mat)
            # losses_dict['rawcelossdim2_simmatrix'] = ce_loss
            # #print("ce loss", ce_loss)
