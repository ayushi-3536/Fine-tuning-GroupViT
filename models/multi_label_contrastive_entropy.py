...
class MultiLabelContrastiveEntropy(nn.Module):
    ...
    def encode_image(self, image, text_token=None,*, return_feat=False, return_attn = True, as_dict=False, align=False):
        outs = Result(as_dict)
        ...
        if align:
            ...
            weighted_image_x = weighted_embeddings.squeeze(-1)
            outs.append(weighted_image_x, 'weighted_image_x')
    ...
    def forward_train(self, image, text, use_aligned_cl=False):
        ...
        image_outs = self.encode_image(image, text_outs['text_multi_label_x'], return_feat=True, return_attn=True, as_dict=True, align=use_aligned_cl)
        # [B, C]
        ...
        if use_aligned_cl:
            losses = self.loss(weighted_image_x, text_x)
        else:
        ...
        if self.with_multi_label:
            if use_aligned_cl: 
                weighted_image_x = image_outs['weighted_image_x']
                image_multi_label_x = weighted_image_x.unsqueeze(1)
            else:
