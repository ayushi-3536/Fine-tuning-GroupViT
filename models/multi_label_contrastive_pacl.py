...
from .group_vit_pacl import CrossAttnBlock
...
class MultiLabelContrastive_PACL(nn.Module):
    ...
    def __init__(self,
        ...
        # self.cross_attention = CrossAttnBlock(
        #     dim=256, num_heads=4, mlp_ratio=4, qkv_bias=True, post_norm=True)
    ...
    def encode_image(self, image, text_token=None, *, return_feat=False, as_dict=False, align_image_text=True):
        outs = Result(as_dict)
        img_outs = self.img_encoder(image, return_feat=return_feat, as_dict=True)

        # print("image x shape", image_x.shape)
        # print("text_x shape", text_token.shape)
        
        image_before_avg_pool = self.img_projector(img_outs['before_avg_pool_x'])
        if align_image_text:
            assert text_token is not None
            #For single text
            if text_token.ndim == 2:
                text_token = text_token.unsqueeze(1)
            logit_scale = torch.clamp(self.logit_scale.exp(), max=100)
            sim = (image_before_avg_pool @ text_token.transpose(-2, -1)) * logit_scale
            attn = F.softmax(sim, dim=-2)
            #For multi-label texts
            if text_token.ndim == 3:
                attn = torch.mean(attn, dim=-1, keepdim=True)
            weighted_embeddings = image_before_avg_pool.transpose(-1, -2) @ attn
            image_before_avg_pool = weighted_embeddings.squeeze(-1)
        
        outs.append(img_outs['x'], 'image_x')
        outs.append(image_before_avg_pool, 'before_avg_pool_x')


            # Sum the weighted embeddings along the second dimension
            # summed_embeddings = weighted_embeddings.sum(dim=1)

            # # Compute the average embeddings by dividing the sum by the number of tokens
            # num_tokens = attn.sum(dim=1, keepdim=True)
            # average_embeddings = summed_embeddings / num_tokens[..., None]

            # # Add a new dimension to the average embeddings using unsqueeze()
            # image_x = average_embeddings.unsqueeze(1)
            # image_x = self.cross_attention(image_x, text_token)
            
            # logit_scale = torch.clamp(self.multi_label_logit_scale.exp(), max=100)
            # s_img = torch.matmul(image_x[..., None], text_token[..., None].transpose(-1, -2))
            # #image_x[..., None] @ text_token[..., None].transpose(-1, -2)
            # #s_img_text = (image_x @ text_token.transpose(0, 2, 1)) * logit_scale
            # a_img_text = F.softmax(s_img, dim=-1)
            # image_x = image_x @ a_img_text
    ...
    def forward_train(self, image, text):
        ...
        #patch_text_x = text_x.clone().unsqueeze(1) #.repeat(1, 8, 1)
        
        image_outs = self.encode_image(image, text_outs['text_multi_label_x'], as_dict=True)
        # [B, C]
        # [B, S_2, C]
        image_x = image_outs['image_x']
        image_before_avg = image_outs['before_avg_pool_x']
        # image_x = self.cross_attention(image_x, patch_text_x)

    
        # image_x = self.img_encoder.forward_image_head(image_x)

        #change only contratsive loss and not ml
        losses = self.loss(image_before_avg, text_x)
    ...
    #     # image_outs = self.encode_image(image, text_outs['text_x'], as_dict=True)
    #     image_outs = self.encode_image(image, text_outs['text_multi_label_x'], as_dict=True)
    #     # [B, C]
    #     image_x = image_outs['image_x']
    ...
    def get_text_embeddings(self, text):
        if self.training and not self.debugging:
            return self.generate_text_embeddings(text)
