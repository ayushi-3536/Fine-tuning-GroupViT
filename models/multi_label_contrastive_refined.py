...
class MultiLabelContrastiveRefined(nn.Module):
    ...
    def find_indexes(self, string_list):
        word_indexes = {}
    ...
    def refined_multi_label_loss(self, image_feat, text_feat, text_meta):
        ...
        #flatten the list
        text_meta_flatten = [item for sublist in text_meta for item in sublist]
        indexes_dict = self.find_indexes(text_meta_flatten)
        # print("after flattening text_meta", text_meta_flatten)
        # print("indexes_dict", indexes_dict)
        true_postive_labels_per_img = []
        for img_map in labels_per_img_onehot:
            texts = [text_meta_flatten[i] for i, val in enumerate(img_map) if (val == 1 and not text_meta_flatten[i] == '<PAD>')]
            text_positive = [0] * len(text_meta_flatten)
            for text in texts:
                for index in indexes_dict[text]:
                        text_positive[index] = 1
            if sum(text_positive) > 0:
                text_positive = [round(x/sum(text_positive), 4) for x in text_positive]
            #print("text_positive", text_positive)
        ...
        for idx, text_map in enumerate(labels_per_text_onehot):
            ...
            if text == '<PAD>':
                true_postive_img_per_label.append([0]*batch)
                continue
            #get all the indices in text_meta_flatten which has the same text as text
            matched_texts_indices = indexes_dict[text] 
            img_map_for_text = [0]*batch
            ...
            #print("img_positive", img_positive)
            true_postive_img_per_label.append(img_positive)

        true_positive_img_per_label = torch.tensor(true_postive_img_per_label, dtype=torch.float32).to(text_x.device)
    ...
    def forward_train(self, image, text, text_meta):
        ...
        if self.padtoken is not None and self.multi_label:
        ...
        if self.with_multi_label:
            ...
            losses_dict['refinedmulti_label_loss'] = self.refined_multi_label_loss(image_multi_label_x, text_multi_label_x_masked, text_meta=text_meta) * self.multi_label_loss_weight
