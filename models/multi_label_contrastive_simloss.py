...
class MultiLabelContrastiveSimLoss(nn.Module):
    ...
    def softcrossentropy(self, gvit_sim, dino_sim):
        ...
        print("gvit_sim", gvit_sim.shape)
        print("dino_sim", dino_sim.shape)
        output = loss(F.softmax(gvit_sim, dim=-1), F.softmax(dino_sim, dim=-1))
        print("output soft target loss", output)
        return output
    ...
    def forward_train(self, image, text):
        ...
        sim_loss = self.softcrossentropy(gvit_sim_mat, dino_sim_mat)
        losses_dict['celoss_sim'] = sim_loss
