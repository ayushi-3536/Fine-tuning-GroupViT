...
class GroupViTTrainAssessment(EncoderDecoder):
    ...
    def get_attn_maps(self, img, return_onehot=False, rescale=False):
        ...
        with torch.no_grad():
            ...
            for idx, attn_dict in enumerate(results['attn_dicts']):
                ...
                group_feats.append(attn_dict['group_feat'])
    ...
    def get_all_feat(self,img_tensor, img_show):
            ...
            attn_map, group_feat = self.get_attn_maps(img_tensor)[-1]
            print("group feat", group_feat)
            ...
            result_dict = {'grouped_image': group_result.squeeze(0), 'grouped_img_tokens': grouped_img_tokens, 'group_feat':group_feat.squeeze(0)}
            return result_dict
    ...
    def encode_decode(self,img, img_metas):
        ...
        attn_map, _ = self.get_attn_maps(img, rescale=True)
        attn_map = attn_map[-1]
        ...
        #print("softmax avg_affinity_mat affinity map", avg_affinity_mat)
        affinity_mask = torch.zeros_like(avg_affinity_mat)
        avg_affinity_topk = avg_affinity_mat.topk(dim=-1, k=min(5, num_labels))
        print("avg_affinity_mat  topk", avg_affinity_topk)
